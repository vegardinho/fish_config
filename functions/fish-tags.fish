...
function fish-tags $argv
    ...
    if [ $ret_val -eq 50 ]
        set_color yellow
        echo "Successfully deleted tags "(set_color green)(cat /tmp/fish_tags.txt)(set_color yellow)" for directory "(set_color green)"\""$PWD"\""(set_color yellow)"."
        return
    else if [ $ret_val -eq 20 ]
        # Get list of tags from tmp file (stored by python script)
        echo (set_color yellow)"Fish tags "(set_color green)(cat /tmp/fish_tags.txt)(set_color yellow)" stored for directory"\
        ...
        set_color yellow
        echo "Successfully added tag(s) "(set_color green)(cat /tmp/fish_tags.txt) (set_color yellow)for folder (set_color green)$PWD
        return
    else
        set_color red
